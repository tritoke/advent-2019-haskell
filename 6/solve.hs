...
indirect orbitmap key depth
  ...
  | HMap.member key orbitmap = depth + (sum . map (\x -> indirect orbitmap x (succ depth))) next_keys
  | otherwise                = depth
  where next_keys = orbitmap HMap.! key

path_from :: HashMap String [String] -> String -> String -> [String]
path_from orbitmap start end
  | not (HMap.member start orbitmap) = []
...
main = do
  ...
  let orbit_map = HMap.fromListWith (++) [ (k, [v]) | [k, v] <- orbits ]
